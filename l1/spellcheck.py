...
from common import d_lev_d, jac_d
...
def spell(str1, dic, ret=5, dist=0.8, steps=0):
    ...
    Args:
        ...
        dist (float, optional): Normalized allowed distance between hashes
                                computed by Jaccard distance. Defaults to 0.8.
        steps (int, optional): Allowed distance in number of different characters
                               in an input string. Defaults to 0.
    ...
    for key in dic:
        if jac_d(key, str1) > dist:
            for word in dic[key]:
                ...
                table['distance'].append(d_lev_d(str1, word))
    return pd.DataFrame(table).sort_values('distance', ascending=False).head(ret)
    ...
    for key in dic:
        if jac_d(key, str1) > dist:
            for word in dic[key]:
                ...
                table['distance'].append((1-freq_imp)*d_lev_d(str1, word[0])[0]
                                         +freq_imp*word[1])
    return pd.DataFrame(table).sort_values('distance', ascending=False).head(ret)


if __name__ == '__main__':
    with open('l1/dictionaries/words_freq_dict.json', 'r', encoding='utf-8') as json_file:
    ...
    # with open('l1/dictionaries/words_rand.json', 'r', encoding='utf-8') as json_file:
    #     word_dict = json.load(json_file)
    # while 1:
    #     try:
    #         inp = input()
    #         print(spell(inp, word_dict, steps=2))
    #     except KeyboardInterrupt:
    #         break
